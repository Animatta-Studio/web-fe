  ...
  return (
    <div className="h-full flex flex-col mt-[100px] items-center">
      ...
      <motion.div initial={{y: '100vw'}} animate={{y: 0}} transition={{type:'spring', duration:1.5, bounce:0.2}} className='flex flex-col items-center z-[8]'>
        <div
          className="flex bg-[#64C0FF] msm:max-w-[900px] max-w-[360px] max-h-[80px] items-center justify-center px-6 msm:mt-[40px] rounded-t-[15px] msm:border-t-4 border-t-2 border-gray-900"
        >
          <ul className="flex relative">
            <span
              className={`bg-[#0087E0] duration-500 ${abroadHeader[selected].dis} ${abroadHeader[selected].sd} msm:border-t-4 border-t-2 msm:border-l-4 border-l-2 msm:border-r-4 border-r-2 border-gray-900 msm:h-[95px] h-[70px] msm:w-[180px] w-[72px] absolute msm:-top-5 -top-6 msm:rounded-t-[30px] rounded-t-[25px] msm:z-0 z-[10]`}
            >
              <span className="bg-[#0087E0] absolute h-1/4 -bottom-1 msm:w-[172px] w-[68px] z-[11]" />
              <span
            ...
            {abroadHeader.map((country, idx) => (
              <li
                key={idx}
                className={`duration-[800ms] msm:w-[180px] w-[72px] ${idx !== 0 && 'msm:border-l-[2px] border-l-[1px]'} ${idx !== abroadHeader.length-1 && 'msm:border-r-[2px] border-r-[1px]'} border-gray-900 pb-2`}
              >
                <a 
