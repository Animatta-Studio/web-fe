    ...
    <div className="h-full flex flex-col mt-[100px] items-center">
      ...
      <div className="flex space-x-5">
        <motion.div initial={{y: '70vh'}} animate={{y: 0}} transition={{type:'spring', duration:1.3, bounce:0.3}} className='msm:w-2/3 flex flex-col items-center z-[8]'>
          <div
            className="flex bg-[#64C0FF] msm:max-w-[640px] max-w-[285px] max-h-[80px] items-center justify-center px-6 msm:mt-[40px] rounded-t-[15px] msm:border-t-4 border-t-2 border-gray-900"
          >
            <ul className="flex relative">
              <span
                className={`bg-[#0087E0] duration-500 ${courses[selected].dis} ${courses[selected].sd} msm:border-t-4 border-t-2 msm:border-l-4 border-l-2 msm:border-r-4 border-r-2 border-gray-900 msm:h-[95px] h-[70px] msm:w-[160px] w-[72px] absolute msm:-top-5 -top-6 msm:rounded-t-[30px] rounded-t-[25px] msm:z-0 z-[10]`}
              >
              ...
              {courses.map((country, idx) => (
                <li
                  key={idx}
                  className={`duration-[800ms] msm:w-[160px] w-[72px] ${idx !== 0 && 'msm:border-l-[2px] border-l-[1px]'} ${idx !== courses.length-1 && 'msm:border-r-[2px] border-r-[1px]'} border-gray-900 pb-2`}
                >
                  <a 
                ...
                <AnimatePresence initial={false} custom={direction}>
                  ...
                  >
                    <div className="flex flex-1 justify-center items-center msm:mb-10 mb-4">
                      <div className="text-white font-poppins msm:text-[18px] text-[14px] text-start msm:leading-[40px] msm:px-[1px] px-0">
                        {parse(courses[selected].content)}
                      </div>
                    </div>
                  </motion.div>
                </AnimatePresence>
        ...
        <motion.img src={courses[selected].icon} loading='eager' className="msm:inline hidden h-[400px] w-[400px] relative top-[70px]" />
      </div>
    </div>
  );
};
